...
<style>
    ...
    /* ═══ ZOYA-STYLE STOCK PAGE ═══ */

    /* Big Verdict Banner */
    .verdict-banner {
        text-align: center; padding: 28px 20px; border-radius: 18px; margin: 14px 0;
        position: relative; overflow: hidden;
    }
    .verdict-banner::before {
        content: ''; position: absolute; top: 0; left: 0; right: 0; bottom: 0;
        background: repeating-conic-gradient(transparent 0% 25%, rgba(255,255,255,0.02) 0% 50%) 50% / 40px 40px;
    }
    .verdict-banner * { position: relative; }
    .vb-halal { background: linear-gradient(135deg, #E6F3ED 0%, #D1EDDF 100%); border: 1.5px solid rgba(45,122,95,0.2); }
    .vb-haram { background: linear-gradient(135deg, #FEF2F0 0%, #FCDBD7 100%); border: 1.5px solid rgba(196,50,26,0.2); }
    .vb-nc { background: linear-gradient(135deg, #FEF7ED 0%, #FDECD3 100%); border: 1.5px solid rgba(180,83,9,0.2); }
    .vb-nd { background: linear-gradient(135deg, #F5F5F3 0%, #ECECEA 100%); border: 1px solid var(--warm-200); }
    .vb-icon { font-size: 2.2rem; margin-bottom: 6px; }
    .vb-status { font-size: 1.5rem; font-weight: 800; letter-spacing: -0.5px; margin: 0; }
    .vb-halal .vb-status { color: #1A6B4A; }
    .vb-haram .vb-status { color: #A12A1A; }
    .vb-nc .vb-status { color: #8B5A08; }
    .vb-nd .vb-status { color: var(--stone); }
    .vb-sub { font-size: 0.78rem; margin: 4px 0 0; opacity: 0.7; }
    .vb-halal .vb-sub { color: #1A6B4A; }
    .vb-haram .vb-sub { color: #A12A1A; }
    .vb-nc .vb-sub { color: #8B5A08; }
    ...
    .stock-name { font-size: 1.5rem; font-weight: 800; color: var(--ink); margin: 0; letter-spacing: -0.5px; }
    .stock-meta { font-size: 0.78rem; color: var(--ink-3); margin: 4px 0 0; }
    .stock-price { text-align: right; }
    .stock-price .price { font-family: 'Source Code Pro', monospace; font-size: 1.8rem; font-weight: 700; color: var(--ink); }
    .stock-price .price-label { font-size: 0.65rem; color: var(--ink-4); text-transform: uppercase; letter-spacing: 0.5px; }
    ...
    .screen-title {
        ...
        padding-bottom: 10px; border-bottom: 1px solid var(--warm-100);
    }
    .screen-title-icon {
        width: 32px; height: 32px; border-radius: 8px; display: flex;
        align-items: center; justify-content: center; font-size: 0.9rem; flex-shrink: 0;
    }
    .sti-green { background: var(--sage-light); }
    .sti-blue { background: var(--sky-light); }
    .sti-amber { background: var(--honey-light); }
    .sti-purple { background: #EEEDFE; }
    .screen-title h4 { font-size: 0.9rem; font-weight: 700; color: var(--ink); margin: 0; }
    .screen-title .screen-verdict {
        margin-left: auto; font-size: 0.68rem; font-weight: 700; padding: 3px 12px;
        border-radius: 20px; letter-spacing: 0.3px;
    }
    .sv-pass { background: var(--sage-light); color: var(--sage); }
    .sv-fail { background: #FEF2F0; color: var(--coral); }
    .sv-warn { background: var(--honey-light); color: var(--honey); }

    /* Ratio Row (Zoya-style pass/fail) */
    .ratio-row {
        display: flex; align-items: center; gap: 12px; padding: 10px 0;
        border-bottom: 1px solid var(--warm-100);
    }
    .ratio-row:last-child { border-bottom: none; }
    .ratio-icon { font-size: 1rem; flex-shrink: 0; width: 24px; text-align: center; }
    .ratio-info { flex: 1; }
    .ratio-name { font-size: 0.78rem; font-weight: 600; color: var(--ink); margin: 0; }
    .ratio-detail { font-size: 0.68rem; color: var(--ink-3); margin: 2px 0 0; }
    .ratio-bar-wrap { width: 120px; flex-shrink: 0; }
    .ratio-bar { height: 6px; background: var(--warm-100); border-radius: 3px; overflow: hidden; }
    .ratio-bar-fill { height: 100%; border-radius: 3px; transition: width 0.5s; }
    .ratio-val {
        font-family: 'Source Code Pro', monospace; font-size: 0.78rem; font-weight: 700;
    ...
    .rb-pass { background: var(--sage-light); color: var(--sage); }
    .rb-fail { background: #FEF2F0; color: var(--coral); }
    .rb-na { background: var(--warm-100); color: var(--ink-4); }

    /* Company Info Card */
    .company-desc {
        font-size: 0.8rem; color: var(--ink-2); line-height: 1.7; margin: 0;
        max-height: 120px; overflow: hidden; transition: max-height 0.3s;
    }
    .company-detail {
        display: flex; gap: 8px; align-items: center; padding: 6px 0;
        border-bottom: 1px solid var(--warm-100); font-size: 0.78rem;
    }
    .company-detail:last-child { border-bottom: none; }
    .cd-label { color: var(--ink-3); min-width: 100px; font-weight: 500; }
    .cd-value { color: var(--ink); font-weight: 600; }
    ...
    .sc-num { font-family: 'Source Code Pro', monospace; font-size: 1.5rem; font-weight: 700; line-height: 1; }
    .sc-label { font-size: 0.5rem; color: var(--ink-4); text-transform: uppercase; letter-spacing: 0.5px; margin-top: 2px; }

    /* Alternative stocks */
    .alt-chip {
        display: inline-flex; align-items: center; gap: 4px; padding: 6px 14px;
        background: var(--sage-light); border: 1px solid rgba(45,122,95,0.15);
        border-radius: var(--r-full); font-size: 0.72rem; font-weight: 600; color: var(--sage);
        margin: 3px; transition: all 0.15s;
    }
    .alt-chip:hover { background: rgba(45,122,95,0.15); }
</style>
        ...
        with st.spinner(f"Analyse de {tc}..."):
            try:
                ...
                if not inf or inf.get("quoteType")=="NONE":
                    ...
                    # ── STOCK HEADER ──
                    prix_display = f"{r['Prix']} €" if r['Prix'] else "—"
                    st.markdown(f"""
                    <div class="stock-header">
                        <div>
                            <p class="stock-name">{r['Nom']}</p>
                            <p class="stock-meta">{tc} · {r['Secteur']} · {r['Industrie']} · Cap. {r.get('Cap.type','—')}</p>
                        </div>
                        <div class="stock-price">
                            <div class="price">{prix_display}</div>
                            <div class="price-label">Prix actuel</div>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)

                    # ── BIG VERDICT BANNER (Zoya-style) ──
                    vb_map = {
                        "Conforme": ("vb-halal", "✅", "HALAL — CONFORME SHARIA", "Cette action respecte tous les criteres AAOIFI"),
                        "Non conforme": ("vb-nc", "⚠️", "NON CONFORME", "Cette action ne respecte pas un ou plusieurs criteres AAOIFI"),
                        "Haram": ("vb-haram", "🚫", "HARAM — NON AUTORISE", "Cette action est interdite selon les normes de finance islamique"),
                        "Donnees insuff.": ("vb-nd", "📊", "DONNEES INSUFFISANTES", "Pas assez de donnees pour evaluer la conformite"),
                    }
                    vb_cls, vb_icon, vb_text, vb_sub = vb_map.get(r["Statut"], ("vb-nd", "❓", r["Statut"], ""))
                    st.markdown(f"""
                    <div class="verdict-banner {vb_cls}">
                        <div class="vb-icon">{vb_icon}</div>
                        <p class="vb-status">{vb_text}</p>
                        <p class="vb-sub">{vb_sub}</p>
                    </div>
                    """, unsafe_allow_html=True)

                    # ── SCORE CIRCLE (if applicable) ──
                    if r["Score"] > 0:
                        sc = r["Score"]
                        sc_color = "var(--sage)" if sc >= 70 else ("var(--honey)" if sc >= 40 else "var(--coral)")
                        sc_border_color = "#2D7A5F" if sc >= 70 else ("#B8860B" if sc >= 40 else "#C4321A")
                        col_score, col_info = st.columns([1, 3])
                        with col_score:
                            st.markdown(f"""
                            <div class="score-circle" style="border-color:{sc_border_color};">
                                <div class="sc-num" style="color:{sc_color};">{int(sc)}</div>
                                <div class="sc-label">Score</div>
                            </div>
                            <div style="position:absolute;inset:0;border-radius:50%;border:4px solid {sc_border_color};"></div>
                            """, unsafe_allow_html=True)
                        with col_info:
                            st.markdown(f"""
                            <div style="padding:8px 0;">
                                <p style="font-size:0.85rem;font-weight:700;color:var(--ink);margin:0;">Score Sharia : {int(sc)}/100</p>
                                <p style="font-size:0.75rem;color:var(--ink-3);margin:4px 0 0;line-height:1.5;">
                                    Moyenne des 5 ratios AAOIFI inverses. Plus le score est eleve, plus l'action est eloignee des seuils d'exclusion.
                                </p>
                            </div>
                            """, unsafe_allow_html=True)
                    ...
                    # Section 1: Business Activity Screening
                    bus_pass = r["Statut"] != "Haram" or r.get("Niveaux","") in ["N5","N5b","N6","N6b"]
                    bus_verdict_cls = "sv-pass" if r.get("Niveaux","") not in ["N1","N2","N3","N4","N4b"] else "sv-fail"
                    bus_verdict_text = "PASS" if bus_verdict_cls == "sv-pass" else "FAIL"

                    bus_html = f"""<div class="screen-card">
                        <div class="screen-title">
                            <div class="screen-title-icon sti-green">🏢</div>
                            <h4>Business Activity Screening</h4>
                            <span class="screen-verdict {bus_verdict_cls}">{bus_verdict_text}</span>
                        </div>"""

                    # Sector check
                    sec_ok = r["Secteur"] not in SECTEURS_HARAM
                    bus_html += f"""<div class="ratio-row">
                        <span class="ratio-icon">{"✅" if sec_ok else "❌"}</span>
                        <div class="ratio-info"><p class="ratio-name">Secteur d'activite</p>
                        <p class="ratio-detail">{r['Secteur']} — {"Secteur autorise" if sec_ok else "Secteur interdit (Finance)"}</p></div>
                        <span class="ratio-badge {"rb-pass" if sec_ok else "rb-fail"}">{"PASS" if sec_ok else "FAIL"}</span>
                    </div>"""

                    # Industry check
                    ind_ok = r["Industrie"] not in INDUSTRIES_HARAM
                    bus_html += f"""<div class="ratio-row">
                        <span class="ratio-icon">{"✅" if ind_ok else "❌"}</span>
                        <div class="ratio-info"><p class="ratio-name">Industrie</p>
                        <p class="ratio-detail">{r['Industrie']} — {"Industrie autorisee" if ind_ok else "Industrie interdite"}</p></div>
                        <span class="ratio-badge {"rb-pass" if ind_ok else "rb-fail"}">{"PASS" if ind_ok else "FAIL"}</span>
                    </div>"""

                    # Blacklist check
                    bl_ok = tc not in LISTE_NOIRE
                    bus_html += f"""<div class="ratio-row">
                        <span class="ratio-icon">{"✅" if bl_ok else "❌"}</span>
                        <div class="ratio-info"><p class="ratio-name">Liste noire</p>
                        <p class="ratio-detail">{"Non presente dans la liste noire" if bl_ok else LISTE_NOIRE.get(tc, "Entreprise exclue manuellement")}</p></div>
                        <span class="ratio-badge {"rb-pass" if bl_ok else "rb-fail"}">{"PASS" if bl_ok else "FAIL"}</span>
                    </div>"""

                    # Keywords check
                    kw_result = ckw(inf)
                    kw_ok = len(kw_result) == 0
                    bus_html += f"""<div class="ratio-row">
                        <span class="ratio-icon">{"✅" if kw_ok else "❌"}</span>
                        <div class="ratio-info"><p class="ratio-name">Activites haram detectees</p>
                        <p class="ratio-detail">{"Aucun mot-cle haram dans la description" if kw_ok else f"Termes detectes : {', '.join(kw_result[:4])}"}</p></div>
                        <span class="ratio-badge {"rb-pass" if kw_ok else "rb-fail"}">{"PASS" if kw_ok else "FAIL"}</span>
                    </div>"""

                    # Zone grise
                    grey_zone = r["Industrie"] in INDUSTRIES_ATTENTION
                    if grey_zone:
                        bus_html += f"""<div class="ratio-row">
                            <span class="ratio-icon">⚠️</span>
                            <div class="ratio-info"><p class="ratio-name">Zone grise</p>
                            <p class="ratio-detail">Industrie a surveiller : {r['Industrie']} (malus -10 pts)</p></div>
                            <span class="ratio-badge sv-warn">ATTENTION</span>
                        </div>"""

                    bus_html += "</div>"
                    st.markdown(bus_html, unsafe_allow_html=True)

                    # Section 2: Financial Screening (only if not already Haram from business screening)
                    if r["Statut"] not in ["Haram", "Donnees insuff."] or r.get("Niveaux","") in ["N5","N5b","N6","N6b"]:
                        fin_fails = [n for n in r.get("Niveaux","").split(", ") if n.startswith("N5") or n.startswith("N6")]
                        fin_verdict_cls = "sv-fail" if fin_fails else "sv-pass"
                        fin_verdict_text = "FAIL" if fin_fails else "PASS"

                        def ratio_row_html(name, detail, value, seuil, unit="%"):
                            if not isinstance(value, (int, float)):
                                return f"""<div class="ratio-row">
                                    <span class="ratio-icon">➖</span>
                                    <div class="ratio-info"><p class="ratio-name">{name}</p><p class="ratio-detail">{detail}</p></div>
                                    <div class="ratio-bar-wrap"><div class="ratio-bar"><div class="ratio-bar-fill" style="width:0%;"></div></div></div>
                                    <span class="ratio-val" style="color:var(--ink-4);">—</span>
                                    <span class="ratio-badge rb-na">N/A</span></div>"""
                            is_ok = value <= seuil
                            pct = min(100, value / seuil * 100) if seuil > 0 else 0
                            color = "#2D7A5F" if value <= seuil * 0.6 else ("#B8860B" if is_ok else "#C4321A")
                            icon = "✅" if is_ok else "❌"
                            badge_cls = "rb-pass" if is_ok else "rb-fail"
                            badge_text = "PASS" if is_ok else "FAIL"
                            return f"""<div class="ratio-row">
                                <span class="ratio-icon">{icon}</span>
                                <div class="ratio-info"><p class="ratio-name">{name}</p><p class="ratio-detail">{detail}</p></div>
                                <div class="ratio-bar-wrap"><div class="ratio-bar"><div class="ratio-bar-fill" style="width:{pct}%;background:{color};"></div></div></div>
                                <span class="ratio-val" style="color:{color};">{value}{unit}</span>
                                <span class="ratio-badge {badge_cls}">{badge_text}</span></div>"""

                        fin_html = f"""<div class="screen-card">
                            <div class="screen-title">
                                <div class="screen-title-icon sti-blue">📊</div>
                                <h4>Financial Screening — Ratios AAOIFI</h4>
                                <span class="screen-verdict {fin_verdict_cls}">{fin_verdict_text}</span>
                            </div>"""
                        fin_html += ratio_row_html("Dette / Capitalisation", f"Dette totale rapportee a la capitalisation boursiere moyenne 12 mois. Seuil : {SEUIL_DETTE}%", r.get("Dette/Cap(%)"), SEUIL_DETTE)
                        fin_html += ratio_row_html("Cash + Investissements / Capitalisation", f"Tresorerie et placements rapportes a la capitalisation. Seuil : {SEUIL_CASH}%", r.get("Cash+Inv/Cap(%)"), SEUIL_CASH)
                        fin_html += ratio_row_html("Creances / Capitalisation", f"Creances clients rapportees a la capitalisation. Seuil : {SEUIL_CREANCES}%", r.get("Creances/Cap(%)"), SEUIL_CREANCES)
                        fin_html += ratio_row_html("Revenus haram / Chiffre d'affaires", f"Interets et autres revenus non conformes. Seuil : {SEUIL_REV_HARAM}%", r.get("Rev.haram(%)"), SEUIL_REV_HARAM)
                        fin_html += ratio_row_html("Charges d'interets / Chiffre d'affaires", f"Charges financieres conventionnelles. Seuil : {SEUIL_INT_EXP}%", r.get("Charges int.(%)"), SEUIL_INT_EXP)

                        # D/E supplementary
                        de_val = r.get("D/E")
                        if isinstance(de_val, (int, float)):
                            de_ok = de_val <= SEUIL_DE
                            de_color = "#2D7A5F" if de_ok else "#C4321A"
                            fin_html += f"""<div class="ratio-row">
                                <span class="ratio-icon">{"✅" if de_ok else "❌"}</span>
                                <div class="ratio-info"><p class="ratio-name">Debt-to-Equity (complementaire)</p><p class="ratio-detail">Ratio d'endettement. Seuil : {SEUIL_DE}</p></div>
                                <div class="ratio-bar-wrap"><div class="ratio-bar"><div class="ratio-bar-fill" style="width:{min(100,de_val/SEUIL_DE*100)}%;background:{de_color};"></div></div></div>
                                <span class="ratio-val" style="color:{de_color};">{de_val}</span>
                                <span class="ratio-badge {"rb-pass" if de_ok else "rb-fail"}">{"PASS" if de_ok else "FAIL"}</span></div>"""
                    ...
                    if r.get("Purif.(%)") and r["Purif.(%)"] != "—" and r["Purif.(%)"] > 0:
                        st.markdown(f"""<div class="screen-card">
                            <div class="screen-title">
                                <div class="screen-title-icon sti-purple">🕌</div>
                                <h4>Purification du dividende</h4>
                            </div>
                            <div class="ratio-row">
                                <span class="ratio-icon">💰</span>
                                <div class="ratio-info">
                                    <p class="ratio-name">Taux de purification : {r["Purif.(%)"]}%</p>
                                    <p class="ratio-detail">Pour chaque 100€ de dividendes recus, vous devez reverser {r["Purif.(%)"]}€ en sadaqa (charite) pour purifier votre investissement. Ce montant correspond a la part des revenus non conformes (interets) dans le chiffre d'affaires de l'entreprise.</p>
                                </div>
                            </div>
                        </div>""", unsafe_allow_html=True)
                    ...
                    # ── COMPANY INFO ──
                    company_desc = inf.get("longBusinessSummary", "")
                    company_employees = inf.get("fullTimeEmployees")
                    company_website = inf.get("website", "")
                    company_country = inf.get("country", "")
                    high52 = inf.get("fiftyTwoWeekHigh")
                    low52 = inf.get("fiftyTwoWeekLow")
                    avg_vol = inf.get("averageVolume")

                    if company_desc or company_employees or company_website:
                        info_html = f"""<div class="screen-card">
                            <div class="screen-title">
                                <div class="screen-title-icon sti-amber">🏛️</div>
                                <h4>A propos de {r['Nom']}</h4>
                            </div>"""
                        if company_desc:
                            short_desc = company_desc[:400] + "..." if len(company_desc) > 400 else company_desc
                            info_html += f'<p class="company-desc">{short_desc}</p>'
                        info_html += '<div style="margin-top:14px;">'
                        if r['Secteur'] != "Inconnu":
                            info_html += f'<div class="company-detail"><span class="cd-label">Secteur</span><span class="cd-value">{r["Secteur"]}</span></div>'
                        if r['Industrie'] != "Inconnu":
                            info_html += f'<div class="company-detail"><span class="cd-label">Industrie</span><span class="cd-value">{r["Industrie"]}</span></div>'
                        if company_country:
                            info_html += f'<div class="company-detail"><span class="cd-label">Pays</span><span class="cd-value">{company_country}</span></div>'
                        if company_employees:
                            info_html += f'<div class="company-detail"><span class="cd-label">Employes</span><span class="cd-value">{company_employees:,}</span></div>'
                        if company_website:
                            info_html += f'<div class="company-detail"><span class="cd-label">Site web</span><span class="cd-value">{company_website}</span></div>'
                        if high52 and low52:
                            info_html += f'<div class="company-detail"><span class="cd-label">52 semaines</span><span class="cd-value">{round(low52,2)} — {round(high52,2)} €</span></div>'
                        if avg_vol:
                            info_html += f'<div class="company-detail"><span class="cd-label">Vol. moyen</span><span class="cd-value">{avg_vol:,}</span></div>'
                        info_html += '</div></div>'
                        st.markdown(info_html, unsafe_allow_html=True)

                    # ── HALAL ALTERNATIVES (if Haram) ──
                    if r["Statut"] == "Haram":
                        # Find conformes in same sector from TICKERS
                        same_sector = [nm for tk_alt, nm in TICKERS.items() if tk_alt != tc][:8]
                        st.markdown(f"""<div class="screen-card">
                            <div class="screen-title">
                                <div class="screen-title-icon sti-green">🔄</div>
                                <h4>Explorez des alternatives halal</h4>
                            </div>
                            <p style="font-size:0.78rem;color:var(--ink-3);margin:0 0 10px;">Cette action n'est pas conforme. Utilisez l'onglet Screener pour decouvrir les actions Euronext conformes Sharia, ou recherchez un autre ticker ci-dessus.</p>
                        </div>""", unsafe_allow_html=True)
